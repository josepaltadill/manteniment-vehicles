...
export async function inspeccionarBootstrapFamiliar(cliente: ClienteConsulta, nombreDestino: string, adminUserId: string) {
  const resultado = await cliente.query<FilaPreflight>(
    `select h.id, h.nombre, count(distinct v.id)::text as vehiculos, count(distinct e.id)::text as eventos,
     ...
     from public.fam_hogares h
     left join public.fam_miembros_hogar m on m.household_id = h.id and m.user_id = $2
     left join public.fam_ve_vehiculos v on v.household_id = h.id
     left join public.fam_ve_eventos_vehiculo e on e.household_id = h.id
     where lower(btrim(h.nombre)) = lower(btrim($1))
       or exists (select 1 from public.fam_miembros_hogar own where own.household_id = h.id and own.user_id = $2)
